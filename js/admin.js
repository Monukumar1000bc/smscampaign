function SMSAlert_change_nav(e, t) { if (jQuery(e).attr('tab_type') == 'callbacks' && jQuery('#default_country_code > option').length < 2) { loadCountry(); } return jQuery(".SMSAlert_nav_tabs li").removeClass("SMSAlert_active"), jQuery(e).addClass("SMSAlert_active"), jQuery(".SMSAlert_nav_box").removeClass("SMSAlert_active"), jQuery("." + t).addClass("SMSAlert_active"), !1 }

function disabledMessage() { for (var e = jQuery('.SMSAlert_box input[type="checkbox"]').length, t = 0; e > t; t++) !1 === jQuery('.SMSAlert_box input[type="checkbox"]').eq(t).is(":checked") ? jQuery(jQuery(this).parent().attr("data-href")).find("textarea").attr("readonly", !0) : jQuery(jQuery(this).parent().attr("data-href")).find("textarea").removeAttr("readonly") }

function verifyUser(e) {
    var t = jQuery('input[data-id="smsalert_name"]').val(),
        s = jQuery('input[data-id="smsalert_password"]').val(),
        i = jQuery(e).text();
    jQuery(e).text("Loading..."), "" == t || "" == s ? (jQuery("#verify_status").html("<strong>Please Enter Username or Password.</strong>").fadeOut(3e3, function() { jQuery("#verify_status").html(""), jQuery("#verify_status").removeAttr("style") }), jQuery(e).text(i)) : jQuery.ajax({
        url: "admin.php",
        type: "GET",
        data: "option=smsalert-woocommerce-senderlist&user=" + encodeURIComponent(t) + "&pwd=" + encodeURIComponent(s),
        crossDomain: !0,
        dataType: "json",
        ...
        success: function(t) {
            if ("object" == typeof t) var s = t;
                ...
                if (r.length > 0) {
                    t = r.shift();
                    s = jQuery.parseJSON(t)
                } else jQuery("#verify_status").html("<strong>Response Error</strong>").fadeOut(3e3, function() { jQuery("#verify_status").html(""), jQuery("#verify_status").removeAttr("style") })
            }
            "success" == s.status || "no senderid available for your account" == s.description.desc ? (jQuery('select[id="smsalert_gateway[smsalert_api]"]').removeAttr("disabled"), jQuery('select[id="smsalert_gateway[smsalert_api]"] option').remove(), 0 == s.description.length || "no senderid available for your account" == s.description.desc ? jQuery('select[id="smsalert_gateway[smsalert_api]"]').html("<option value='ESTORE'>ESTORE</option>") : jQuery.each(s.description, function(e, t) {
                (t.Senderid.approved) ? jQuery('select[id="smsalert_gateway[smsalert_api]"]').append(jQuery("<option></option>").attr("value", t.Senderid.sender).text(t.Senderid.sender)): ""
            }), ((jQuery('select[id="smsalert_gateway[smsalert_api]"] option').length == 0 || jQuery('select[id="smsalert_gateway[smsalert_api]"] option').val() == "SELECT") ? jQuery('select[id="smsalert_gateway[smsalert_api]"]').html("<option value='ESTORE'>ESTORE</option>") : "")) : (jQuery('select[id="smsalert_gateway[smsalert_api]"]').html("<option value='SELECT'>SELECT</option>").attr("disabled", !0), jQuery("#verify_status").html("<strong>" + (void 0 !== s.description.desc ? s.description.desc : s.description) + "</strong>").fadeOut(3e3, function() { jQuery("#verify_status").html(""), jQuery("#verify_status").removeAttr("style") })), jQuery(e).text(i), autoSave(e)
        },
...
function create_group(e) {
    ...
    return jQuery(e).text("Please Wait..."), jQuery.ajax({
        url: "admin.php",
        type: "GET",
        data: "option=smsalert-woocommerce-creategroup&user=" + encodeURIComponent(t) + "&pwd=" + encodeURIComponent(s),
        crossDomain: !0,
        dataType: "json",
        contentType: "application/json; charset=utf-8",
        success: function(t) {
            if ("object" == typeof t) var s = t;
            else s = jQuery.parseJSON(t);
            "success" == s.status && (jQuery('select[id="group_auto_sync"]').removeAttr("disabled"), jQuery('select[id="group_auto_sync"]').html(""), jQuery.each(s.description, function(t, s) { jQuery('select[id="group_auto_sync"]').append(jQuery("<option></option>").attr("value", s.Group.id).text(s.Group.name)), jQuery(e).remove() }))
        },
        error: function(e) { console.log(e) }
    }), !1
}
...
function close_accordion_section() { jQuery(".cvt-accordion-body-title").removeClass("active"), jQuery(".cvt-accordion .cvt-accordion-body-content").slideUp(300).removeClass("open"), jQuery(".expand_btn").removeClass("active") }

function openAccordian(e) {
    var t = jQuery(e).attr("data-href");
    jQuery(e).is(".active") ? close_accordion_section() : (close_accordion_section(), jQuery(e).addClass("active"), jQuery(".cvt-accordion " + t).slideDown(300).addClass("open"), jQuery(".expand_btn", e).addClass("active"))
}

function toggleDisabled(e) {
    var t = jQuery(e).attr("id");
    0 == jQuery(e).is(":checked") || "readonly" == jQuery(e).attr("readonly") ? (jQuery('[data-parent_id="' + t + '"]').trigger("change").attr("readonly", !0).addClass("anchordisabled"), jQuery('[data-parent_id="' + t + '"].chosen-select').attr("disabled", !0)) : jQuery('[data-parent_id="' + t + '"]').removeAttr("disabled").removeAttr("readonly").removeClass("anchordisabled").trigger("change"), jQuery(".chosen-select").trigger("chosen:updated")
}

function moreOptInLabel(e) {
    if (e) {
        jQuery(".SMSAlert_nav_box .tabset tab-panels section:first-child").show();
        var t = jQuery(".SMSAlert_nav_box." + e).width(),
            s = (jQuery(".SMSAlert_nav_box." + e + " .tabset ul li").length, 0);
        jQuery(".SMSAlert_nav_box." + e + " .tabset ul li").not(".more_tab").each(function() {
            if (s += jQuery(this).width(), t < s) {
                var i = jQuery(".SMSAlert_nav_box.SMSAlert_active .tabset ul").children(":not(.more_tab)").slice(5);
                jQuery(".SMSAlert_nav_box." + e + " .more_tab ul").html(i), jQuery(".SMSAlert_nav_box." + e + " .tabset ul li.more_tab").removeClass("hide")
            }
        })
    }
}
jQuery(window).on("load", function(e) {
        ...
        "" != s && "" != t && (jQuery(".SMSAlert_nav_tabs li").removeClass("SMSAlert_active"), jQuery(".SMSAlert_nav_box").removeClass("SMSAlert_active"), jQuery('a[href="' + t + '"]').parent().addClass("SMSAlert_active"), jQuery(".SMSAlert_box ." + s).addClass("SMSAlert_active")), disabledMessage()
    }), jQuery('.SMSAlert_box input[type="checkbox"]').click(function() {!1 === jQuery(this).is(":checked") ? 0 == jQuery(this).parent().find('input[type="checkbox"]:checked').length && jQuery(this).parent().parent().find("textarea").attr("readonly", !0) : jQuery(this).parent().parent().find("textarea").removeAttr("readonly") }), jQuery("#wc_sms_alert_sms_order_message").on("change keyup input", function() { jQuery("#wc_sms_alert_sms_order_message_char_count").text(jQuery(this).val().length), jQuery(this).val().length > 968 ? jQuery("#wc_sms_alert_sms_order_message_char_count").css("color", "red") : jQuery("#wc_sms_alert_sms_order_message_char_count").css("color", "green") }), jQuery("a#wc_sms_alert_sms_order_send_message").click(function(e) {
        var t = jQuery("div#wc_sms_alert_send_sms_meta_box"),
            s = jQuery("textarea#wc_sms_alert_sms_order_message");
        if ($orderid = jQuery("input#wc_sms_alert_order_id"), "" == s.val()) return t.addClass("processing").block({ sms_body: null, timeout: 2e3, message: "Please Enter Your Message.", overlayCSS: { background: "#fff", backgroundSize: "100px 400px", opacity: 1 } }), !1;
        t.addClass("processing").block({ sms_body: null, overlayCSS: { background: "#fff", backgroundSize: "100px 400px", opacity: 1 } });
        var i = { action: "wc_sms_alert_sms_send_order_sms", sms_body: s.val(), order_id: $orderid.val() };
        return jQuery.ajax({
            type: "POST",
            url: smsalert.ajaxurl,
            data: i,
            success: function(e) {
                if (t.removeClass("processing").unblock(), e) {
                    var i = JSON.parse(e),
                        r = "error" == i.status ? i.description.desc : "Sent Successfully.";
                    t.addClass("smsstatus").block({ sms_body: null, timeout: 2e3, message: r, overlayCSS: { background: "#fff", backgroundSize: "100px 400px", opacity: 1 } }), s.val("")
                }
            },
            dataType: "html"
        }), !1
    ...
    jQuery.ajax({
        ...
        success: function(data) {
            ...
            if (data.status == 'success') {
                var mcountry = '';
                jQuery.each(data.description, function(i, item) {
                    ...
                    if (jQuery.inArray(item.Country.c_code, smsalert.whitelist_countries) != -1) {
            ...
            jQuery("#whitelist_country").html(mcountry);
            jQuery("#default_country_code").html(country);
